...
const allowedAffiliateProfile = ["firstName", "lastName", "affiliateName", "email", "mobile", "username", "profileImage", "status", "accessEnabled", "roleId", "referralTarget", "commissionRatePercent", "dateOfBirth", "promotionChannels", "marketingCategory", "experienceLevel", "company", "organization", "profession", "website", "socialMediaLinks", "address", "city", "state", "country", "pincode", "description", "accountHolderName", "bankName", "accountNumber", "ifsc", "swiftCode", "upiId", "paypalEmail", "otherPaymentInformation", "paymentPreference", "pan", "gst", "notes"];
const PAYMENT_STATUSES = ["ACTIVE", "TARGET_REACHED", "ELIGIBLE_FOR_PAYMENT", "ADMIN_REVIEW", "PAYMENT_PROCESSING", "PAYMENT_SENT", "PAYMENT_COMPLETED", "FAILED", "CANCELLED"];
const PAYMENT_TRANSITIONS = { ACTIVE: ["TARGET_REACHED", "CANCELLED"], TARGET_REACHED: ["ELIGIBLE_FOR_PAYMENT", "CANCELLED"], ELIGIBLE_FOR_PAYMENT: ["ADMIN_REVIEW", "PAYMENT_PROCESSING", "CANCELLED"], ADMIN_REVIEW: ["PAYMENT_PROCESSING", "CANCELLED"], PAYMENT_PROCESSING: ["PAYMENT_SENT", "FAILED"], PAYMENT_SENT: ["PAYMENT_COMPLETED", "FAILED"], FAILED: ["PAYMENT_PROCESSING", "CANCELLED"], PAYMENT_COMPLETED: [], CANCELLED: [] };
...
async function syncPaymentCycles(affiliateId) {
  ...
  for (let index = 0; index < cycleCount; index += 1) {
    ...
    await AffiliatePaymentCycle.updateOne({ affiliateId, cycleNumber: index + 1 }, { $set: set, $setOnInsert: { affiliateId, cycleNumber: index + 1, status: eligible ? "TARGET_REACHED" : "ACTIVE", statusHistory: [{ status: eligible ? "TARGET_REACHED" : "ACTIVE", changedAt: new Date(), changedBy: "SYSTEM" }], ...(eligible ? { eligibleAt: slice.at(-1)?.purchaseAt || new Date() } : {}) } }, { upsert: true });
    if (eligible) await AffiliatePaymentCycle.updateOne({ affiliateId, cycleNumber: index + 1, status: "ACTIVE" }, { $set: { status: "TARGET_REACHED", eligibleAt: slice.at(-1)?.purchaseAt || new Date() }, $push: { statusHistory: { status: "TARGET_REACHED", changedAt: new Date(), changedBy: "SYSTEM" } } });
  }
...
affiliateMarketingRouter.post("/affiliate-marketing/affiliates", asyncHandler(async (req, res) => { const body = req.body || {}; if (!body.email || !body.username || !body.password || !body.affiliateName) throw new AppError("Affiliate name, email, username and password are required", 400); if (String(body.password).length < 8) throw new AppError("Password must be at least 8 characters", 400); let code = String(body.affiliateCode || "").trim().toUpperCase(); if (code && !/^[A-Z0-9_-]{4,24}$/.test(code)) throw new AppError("Invalid affiliate code", 400); if (!code) { code = `AFF${String(await Affiliate.countDocuments() + 1).padStart(4, "0")}`; while (await Affiliate.exists({ affiliateCode: code })) code = `AFF${crypto.randomInt(1000, 999999)}`; } await ensureRoles(); const config = await settings(); const defaultRole = config.defaultRoleId || (await AffiliateRole.findOne({ isDefault: true }))?._id; const payload = sanitizeAffiliatePayload(body, true); const affiliate = await Affiliate.create({ ...payload, roleId: payload.roleId || defaultRole, affiliateCode: code, passwordHash: hashPassword(body.password), referralLink: buildReferralLink(config.referralBaseUrl, code) }); await AffiliateAuditLog.create({ adminId: actor(req), affiliateId: affiliate._id, action: "Affiliate Created", newData: toPublicAffiliate(affiliate) }); await recordActivity(req, affiliate._id, "AFFILIATE_CREATED", "AFFILIATES", "Affiliate account created"); res.status(201).json({ success: true, data: toPublicAffiliate(affiliate) }); }));
...
affiliateMarketingRouter.post("/affiliate-marketing/affiliates/:id/regenerate-link", asyncHandler(async (req, res) => { const affiliate = await Affiliate.findById(req.params.id); if (!affiliate) throw new AppError("Affiliate not found", 404); const config = await settings(); affiliate.affiliateCode = `AFF${crypto.randomInt(100000,999999)}`; affiliate.referralLink = buildReferralLink(config.referralBaseUrl, affiliate.affiliateCode); await affiliate.save(); await recordActivity(req, affiliate._id, "REFERRAL_LINK_REGENERATED", "REFERRALS", "Affiliate referral code and link were regenerated"); res.json({ success: true, data: affiliate }); }));
