...
const dailyTestSettingsSchema = new Schema(
  {
    ...
    allowBothExamsSameDay: { type: Boolean, default: false },
    subjectDistribution: {
      NEET: {
        Biology: { type: Number, default: 0, min: 0, max: 200 },
        Chemistry: { type: Number, default: 0, min: 0, max: 200 },
        Physics: { type: Number, default: 0, min: 0, max: 200 },
      },
      JEE: {
        Mathematics: { type: Number, default: 0, min: 0, max: 200 },
        Chemistry: { type: Number, default: 0, min: 0, max: 200 },
        Physics: { type: Number, default: 0, min: 0, max: 200 },
      },
    },
...
dailyTestSettingsSchema.pre("validate", function validateDistribution(next) {
  ...
  const validateSubjectGroup = (group, label) => {
    const values = Object.values(group || {}).map((value) => Number(value || 0));
    const total = values.reduce((sum, value) => sum + value, 0);
    if (total > 0 && total !== Number(this.totalQuestions || 0)) {
      return `${label} subject-wise counts must equal total questions`;
    }
    return "";
  };
  const subjectError = validateSubjectGroup(this.subjectDistribution?.NEET, "NEET") || validateSubjectGroup(this.subjectDistribution?.JEE, "JEE");
  if (subjectError) return next(new Error(subjectError));
