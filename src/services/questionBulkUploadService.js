...
const HEADER_ALIASES = {
  question_id: ["question_id", "questionid", "question id", "id", "_id"],
...
const NUMERIC_LIKE_TYPES = new Set(["FILL_BLANKS", "DESCRIPTIVE", "NUMERICAL"]);
...
function normalizeResponseType(value) {
  ...
  if (["match", "matching", "match_following", "match_the_following", "matrix_match"].includes(normalized)) return "single";
...
function buildUpdateMatchKeyFromRaw(raw = {}) {
  const questionId = normalizeText(raw.questionId);
  if (questionId) return `question-id:${questionId}`;
  const questionText = normalizeKey(raw.question);
...
function resolveHeaderMap(headers, { requireFullTemplate = true } = {}) {
  const normalizedHeaders = headers.map(normalizeHeader);
  const map = {};
  const missing = [];

  for (const required of requireFullTemplate ? REQUIRED_HEADERS : []) {
  ...
  for (const optional of ["question_id", ...REQUIRED_HEADERS]) {
    if (map[optional]) continue;
...
function getPresentFieldsFromRawObject(row = {}) {
  const aliases = Object.entries(HEADER_ALIASES).flatMap(([field, aliasesForField]) =>
    aliasesForField.map((alias) => [normalizeHeader(alias), field]),
  );
  const aliasMap = new Map(aliases);
  return [
    ...new Set(
      Object.entries(row)
        .filter(([key, value]) => !String(key).startsWith("__") && normalizeText(value) !== "")
        .map(([key]) => aliasMap.get(normalizeHeader(key)))
        .filter(Boolean),
    ),
  ];
}

function getPresentFieldsFromHeaderMap(map = {}, row = {}) {
  return [
    ...new Set(
      Object.entries(map)
        .filter(([, header]) => header && normalizeText(row[header]) !== "")
        .map(([field]) => field),
    ),
  ];
}

function markDerivedPresentFields(parsed = {}) {
  const presentFields = new Set(parsed.__presentFields || []);
  [
    ["questionImageUrl", "question_image"],
    ["optionAImageUrl", "option_a_image"],
    ["optionBImageUrl", "option_b_image"],
    ["optionCImageUrl", "option_c_image"],
    ["optionDImageUrl", "option_d_image"],
    ["explanationImageUrl", "explanation_image"],
  ].forEach(([field, canonical]) => {
    if (normalizeText(parsed[field])) presentFields.add(canonical);
  });
  return { ...parsed, __presentFields: [...presentFields] };
}
...
async function parseSheet(sheetFile, { uploadMode = "upload" } = {}) {
  if (!sheetFile) throw new AppError("Spreadsheet file is required", 400);
  const isUpdateMode = normalizeUploadMode(uploadMode) === "update";
  ...
  if (fileName.endsWith(".json") || String(sheetFile.mimetype || "").includes("json")) {
    ...
    return rows.map((row) => markDerivedPresentFields(normalizeMixedMediaFields({
      __rowNumber: row.__rowNumber,
      __presentFields: getPresentFieldsFromRawObject(row),
      questionId: row.questionId ?? row.question_id ?? row.id ?? row._id ?? "",
  ...
  const { map, missing } = resolveHeaderMap(headers, { requireFullTemplate: !isUpdateMode });
  
  if (missing.length) {
    throw new AppError(`Invalid file format / headers not matching. Missing: ${missing.join(", ")}`, 400);
  }
  if (isUpdateMode && !map.question_id && !map.question) {
    throw new AppError("Bulk update file must include Question ID or Question", 400);
  }
  ...
  return rawRows
    ...
    .map((row) => {
      ...
      let parsed = {
        ...
        __presentFields: getPresentFieldsFromHeaderMap(map, row),
        questionId: map.question_id ? row[map.question_id] : "",
        question: map.question ? row[map.question] : "",
        ...
        subject: map.subject ? row[map.subject] : "",
        chapter: map.chapter ? row[map.chapter] : "",
        topic: map.topic ? row[map.topic] : "",
        difficulty: map.difficulty ? row[map.difficulty] : "",
        examType: map.exam_type ? row[map.exam_type] : "",
        year: map.year ? row[map.year] : "",
      ...
      parsed = markDerivedPresentFields(normalizeMixedMediaFields(parsed));
...
function findByIdString(items = [], id) {
  return items.find((item) => String(item._id) === String(id));
}

function existingQuestionToRaw(existing, catalog) {
  const subject = findByIdString(catalog.subjects, existing.subjectId);
  const chapter = findByIdString(catalog.chapters, existing.chapterId);
  const topic = findByIdString(catalog.topics, existing.topicId);
  const difficulty = findByIdString(catalog.difficulties, existing.difficultyId);
  const questionType = findByIdString(catalog.questionTypes, existing.questionTypeId);
  const year = findByIdString(catalog.years, existing.yearId);
  return {
    question: existing.question || "",
    questionImageUrl: existing.questionImageUrl || "",
    optionA: existing.optionA || "",
    optionAImageUrl: existing.optionAImageUrl || "",
    optionB: existing.optionB || "",
    optionBImageUrl: existing.optionBImageUrl || "",
    optionC: existing.optionC || "",
    optionCImageUrl: existing.optionCImageUrl || "",
    optionD: existing.optionD || "",
    optionDImageUrl: existing.optionDImageUrl || "",
    correctAnswer: existing.responseType === "multiple"
      ? (existing.correctOptions || []).join(",")
      : existing.responseType === "numeric"
        ? existing.numericAnswer || ""
        : existing.correctOption || "",
    numericAnswer: existing.numericAnswer || "",
    subject: subject?.name || "",
    chapter: chapter?.name || "",
    topic: topic?.name || "",
    difficulty: difficulty?.key || difficulty?.name || existing.difficulty || "",
    examType: existing.examMode || "",
    year: year?.name || "",
    questionType: questionType?.key || questionType?.name || "MCQ",
    responseType: existing.responseType || "",
    explanation: existing.explanation || "",
    explanationImageUrl: existing.explanationImageUrl || "",
    passage: existing.passage || "",
    conceptTags: Array.isArray(existing.conceptTags) ? existing.conceptTags.join(",") : "",
    hasDiagram: String(Boolean(existing.hasDiagram)),
    isNumerical: String(Boolean(existing.isNumerical)),
    exact: String(Boolean(existing.exact)),
    extraFields: existing.extraFields || {},
  };
}

function mergeRawForUpdate(raw = {}, existing, catalog) {
  const base = existingQuestionToRaw(existing, catalog);
  const presentFields = new Set(raw.__presentFields || []);
  const merged = { ...base, extraFields: { ...(base.extraFields || {}) } };
  const rawFieldByCanonical = {
    question_id: "questionId",
    question: "question",
    question_image: "questionImageUrl",
    option_a: "optionA",
    option_a_image: "optionAImageUrl",
    option_b: "optionB",
    option_b_image: "optionBImageUrl",
    option_c: "optionC",
    option_c_image: "optionCImageUrl",
    option_d: "optionD",
    option_d_image: "optionDImageUrl",
  ...
  for (const [canonical, field] of Object.entries(rawFieldByCanonical)) {
    if (canonical === "question_id") continue;
    if (presentFields.has(canonical) && normalizeText(raw[field]) !== "") merged[field] = raw[field];
  }
  if (presentFields.has("video_url") && normalizeText(raw.videoUrl)) merged.videoUrl = raw.videoUrl;
  if (raw.extraFields && typeof raw.extraFields === "object") {
    merged.extraFields = { ...merged.extraFields, ...raw.extraFields };
  }
  return merged;
}

function buildSparseUpdatePatch(raw = {}, fullPayload = {}) {
  const presentFields = new Set(raw.__presentFields || []);
  const patch = {};
  const include = (fields) => fields.forEach((field) => {
    if (Object.prototype.hasOwnProperty.call(fullPayload, field)) patch[field] = fullPayload[field];
  });

  if (presentFields.has("question")) include(["question"]);
  if (presentFields.has("question_image")) include(["questionImageUrl", "hasDiagram"]);
  if (presentFields.has("option_a")) include(["optionA"]);
  if (presentFields.has("option_a_image")) include(["optionAImageUrl", "hasDiagram"]);
  if (presentFields.has("option_b")) include(["optionB"]);
  if (presentFields.has("option_b_image")) include(["optionBImageUrl", "hasDiagram"]);
  if (presentFields.has("option_c")) include(["optionC"]);
  if (presentFields.has("option_c_image")) include(["optionCImageUrl", "hasDiagram"]);
  if (presentFields.has("option_d")) include(["optionD"]);
  if (presentFields.has("option_d_image")) include(["optionDImageUrl", "hasDiagram"]);
  if (presentFields.has("correct_answer") || presentFields.has("numeric_answer")) include(["correctOption", "correctOptions", "numericAnswer"]);
  if (presentFields.has("subject") || presentFields.has("chapter") || presentFields.has("topic") || presentFields.has("exam_type")) {
    include(["subjectId", "chapterId", "topicId", "examMode", "exam"]);
  }
  if (presentFields.has("year")) include(["yearId"]);
  if (presentFields.has("difficulty")) include(["difficultyId", "difficulty"]);
  if (presentFields.has("question_type")) include(["questionTypeId", "responseType", "isNumerical"]);
  if (presentFields.has("response_type")) include(["responseType", "isNumerical"]);
  if (presentFields.has("explanation") || presentFields.has("video_url")) include(["explanation"]);
  if (presentFields.has("explanation_image")) include(["explanationImageUrl", "hasDiagram"]);
  if (presentFields.has("passage")) include(["passage"]);
  if (presentFields.has("concept_tags")) include(["conceptTags"]);
  if (presentFields.has("has_diagram")) include(["hasDiagram"]);
  if (presentFields.has("is_numerical")) include(["isNumerical"]);
  if (presentFields.has("exact")) include(["exact"]);
  if (raw.extraFields && Object.keys(raw.extraFields).length) {
    patch.extraFields = { ...(fullPayload.extraFields || {}) };
  }
  ...
  if (completenessFields.some((field) => Object.prototype.hasOwnProperty.call(patch, field))) {
    include(["questionStatus", "reviewStatus", "isVisibleToUsers", "uploadWarnings"]);
  }
  return patch;
}

async function findUpdateQuestionMatch(raw = {}, catalog) {
  const questionId = normalizeText(raw.questionId);
  if (questionId) {
    if (!mongoose.isValidObjectId(questionId)) {
      return { errors: ["Invalid Question ID"], existingQuestion: null, matchMode: "question_id" };
    }
    return {
      errors: [],
      existingQuestion: await Question.findById(questionId).lean(),
      matchMode: "question_id",
    };
  }

  const questionText = normalizeText(raw.question);
  if (!questionText) {
    return { errors: ["Missing Question ID or Question"], existingQuestion: null, matchMode: "question" };
  }

  const query = { question: questionText };
  const examType = normalizeExamType(raw.examType);
  if (examType) query.examMode = examType;

  const subject = raw.subject && examType ? findSubject(catalog, raw.subject, examType) : null;
  if (subject) query.subjectId = subject._id;

  const chapter = raw.chapter && subject ? findChapter(catalog, raw.chapter, subject._id) : null;
  if (chapter) query.chapterId = chapter._id;

  const topic = raw.topic && chapter ? findTopic(catalog, raw.topic, chapter._id) : null;
  if (topic) query.topicId = topic._id;

  const matches = await Question.find(query).limit(2).lean();
  if (matches.length > 1) {
    return { errors: ["Question text matched multiple questions; add Question ID"], existingQuestion: null, matchMode: "question" };
  }
  return { errors: [], existingQuestion: matches[0] || null, matchMode: "question" };
}

async function validateUpdateRow(raw, catalog, { createMissingQuestions = false } = {}) {
  const errors = [];
  const warnings = [];
  const match = await findUpdateQuestionMatch(raw, catalog);
  errors.push(...match.errors);
  if (errors.length) return { errors, warnings, payload: null, existingQuestion: null };

  const existingQuestion = match.existingQuestion;
  if (!existingQuestion) {
    if (!createMissingQuestions) {
      const label = match.matchMode === "question_id" ? "Question ID" : "Question text";
      return { errors: [`${label} not found`], warnings, payload: null, existingQuestion: null };
    }
    const createValidation = validateRawRow(raw, catalog);
    return {
      ...createValidation,
      warnings: [...new Set([...createValidation.warnings, "Matched question not found; new question will be created"])],
      existingQuestion: null,
      operation: "create",
    };
  }

  const mergedRaw = mergeRawForUpdate(raw, existingQuestion, catalog);
  const validation = validateRawRow(mergedRaw, catalog);
  const patch = validation.payload ? buildSparseUpdatePatch(raw, validation.payload) : null;
  if (patch && !Object.keys(patch).length) warnings.push("No changed fields detected for matched question");
  return {
    ...validation,
    errors: [...new Set(validation.errors)],
    warnings: [...new Set([...validation.warnings, ...warnings, `Existing question will be updated by ${match.matchMode === "question_id" ? "Question ID" : "Question text"}`])],
    payload: patch,
    fullPayload: validation.payload,
    existingQuestion,
    operation: "update",
  };
}
...
function rowDto(row) {
  return {
    ...
    questionId: row.raw?.questionId || (row.uploadedQuestionId ? String(row.uploadedQuestionId) : ""),
...
async function buildPreview(batchId) {
  ...
  const updateCount = rows.filter((row) => row.operation === "update" && ["valid", "warning", "approved"].includes(row.status)).length;
  const createCount = rows.filter((row) => row.operation !== "update" && ["valid", "warning", "approved"].includes(row.status)).length;
  ...
  await QuestionBulkUploadBatch.findByIdAndUpdate(batchId, {
    ...
    duplicateCount,
    updatedCount: batch?.updatedCount || 0,
    updateCandidateCount: updateCount,
    createCandidateCount: createCount,
    imageCount,
...
async function validateRowsForBatch(batchId, catalog) {
  const batch = await QuestionBulkUploadBatch.findById(batchId).select("uploadMode createMissingQuestions").lean();
  ...
  for (const row of rows) {
    const validation = uploadMode === "update"
      ? await validateUpdateRow(row.raw, catalog, { createMissingQuestions: Boolean(batch?.createMissingQuestions) })
      : validateRawRow(row.raw, catalog);
    const duplicateKey = uploadMode === "update"
      ? buildUpdateMatchKeyFromRaw(row.raw) || `row:${row.rowNumber}`
      : buildDuplicateKeyFromRaw(row.raw);
    ...
    if (seenKeys.has(duplicateKey)) errors.push(uploadMode === "update" ? "Duplicate update match in upload file" : "Duplicate Question in upload file");
    else seenKeys.add(duplicateKey);
    let existingQuestion = null;
    if (uploadMode === "update") {
      existingQuestion = validation.existingQuestion || null;
    } else if (!errors.length && validation.payload) {
    ...
    if (validation.fullPayload) row.payloadFull = validation.fullPayload;
    row.operation = validation.operation === "update" ? "update" : "create";
    if (existingQuestion && !errors.length) {
      row.uploadedQuestionId = existingQuestion._id;
      if (uploadMode !== "update") warnings.push("Existing question exact value will be updated");
    } else if (validation.operation === "create" && !errors.length && mongoose.isValidObjectId(row.raw?.questionId)) {
      row.uploadedQuestionId = row.raw.questionId;
    } else {
...
function getNewColumnStatus(rows = [], batch = {}) {
  if (normalizeUploadMode(batch?.uploadMode) === "update") {
    return { columns: [], totalMissingCount: 0, requiresUpdate: false, updateCompleted: true };
  }
...
async function processApprovalJob(batchId) {
  ...
  let updated = Number(batch.updatedCount || 0);
  let failed = 0;
    ...
    for (const row of chunk) {
      ...
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        try {
          ...
          if (uploadMode === "update" && row.uploadedQuestionId) {
            const existingById = await Question.findById(row.uploadedQuestionId);
            if (existingById) {
              if (!Object.keys(payload).length) {
                row.status = "skipped";
                row.errorMessage = "No changed fields detected for matched question";
                skipped += 1;
                await row.save();
                lastError = null;
                break;
              }
              const previous = existingById.toObject({ depopulate: true });
              Object.assign(existingById, payload);
              await existingById.save();
              await logBulkQuestionEdit(previous, existingById, batch.uploadedBy);
              row.status = "approved";
              row.errorMessage = "";
              updated += 1;
              inserted += 1;
          ...
          const existing = uploadMode === "update" ? null : await Question.findOne(buildDuplicateQuery(payload));
          if (existing) {
  ...
  const updatedRows = await QuestionBulkUploadRow.countDocuments({ batchId, status: "approved", operation: "update" });
  const createdRows = await QuestionBulkUploadRow.countDocuments({ batchId, status: "approved", operation: { $ne: "update" } });
  const duplicateRows = await QuestionBulkUploadRow.countDocuments({ batchId, status: "duplicate" });
  const skippedRows = await QuestionBulkUploadRow.countDocuments({ batchId, status: "skipped" });
  ...
  latestBatch.insertedCount = createdRows;
  latestBatch.updatedCount = uploadMode === "update" ? updatedRows : 0;
  latestBatch.skippedCount = duplicateRows + skippedRows;
  latestBatch.uploadedImageCount = uploadedImageCount;
  latestBatch.failedCount = Number(latestBatch.invalidCount || 0) + failedRows;
  latestBatch.processedCount = approvedRows + failedRows + duplicateRows + skippedRows + Number(latestBatch.invalidCount || 0);
...
export const questionBulkUploadService = {
  async validateFile({ sheetFile, uploadedBy, uploadMode = "upload", createMissingQuestions = false }) {
    const normalizedUploadMode = normalizeUploadMode(uploadMode);
    const parsedRows = await parseSheet(sheetFile, { uploadMode: normalizedUploadMode });
    const batch = await QuestionBulkUploadBatch.create({
      ...
      createMissingQuestions: normalizedUploadMode === "update" && Boolean(createMissingQuestions),
      status: "validating",
    ...
    await QuestionBulkUploadRow.insertMany(parsedRows.map((raw, index) => ({
        ...
        raw,
        payload: {},
        operation: normalizedUploadMode === "update" ? "update" : "create",
  ...
  async getStatus({ batchId }) {
    ...
    const updatedRows = rows.filter((row) => row.status === "approved" && row.operation === "update").length;
    const createdRows = rows.filter((row) => row.status === "approved" && row.operation !== "update").length;
    ...
    const processed = approved + failed + duplicates + skipped;
    ...
    return {
      ...
      successfullyUpdatedRows: updatedRows,
      updatedCount: updatedRows,
      newlyCreatedRows: createdRows,
      ...
      failedRecords: rows
        ...
        .map((row) => ({ row: row.rowNumber, questionId: row.raw?.questionId || "", question: row.question || "", error: row.errorMessage || "" })),
